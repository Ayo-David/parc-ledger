...
export class PostingService {
    ...
    async post(input) {
        validate(input);
        return withTenantTransaction(this.database, input.tenantId, (tx) => this.postInTransaction(tx, input));
    }
    ...
    async postInTransaction(tx, input) {
        ...
        if (existing) {
            if (existing.request_hash !== hash)
                throw new PostingError("IDEMPOTENCY_MISMATCH", "Idempotency key was previously used for a different posting");
            const journal = await tx("journals")
                .where({ transaction_id: existing.id })
                .first();
            if (!journal || existing.status !== "COMPLETED")
                throw new PostingError("POSTING_IN_PROGRESS", "Posting is still processing");
            return {
        ...
        const accountRows = await tx("ledger_accounts")
            .whereIn("id", input.entries.map((e) => e.accountId))
            .where({
            tenant_id: input.tenantId,
            ...
            .forUpdate()
            .select("id", "book_id");
        if (accountRows.length !== new Set(input.entries.map((e) => e.accountId)).size)
            throw new PostingError("ACCOUNT_INVALID", "Every account must be active, tenant-scoped, and use the posting currency");
        const bookId = accountRows[0]?.book_id;
        if (!bookId || accountRows.some((a) => a.book_id !== bookId))
            throw new PostingError("BOOK_MISMATCH", "All posting accounts must belong to one ledger book");
        ...
        if (period !== undefined && period.status !== "OPEN")
            throw new PostingError("ACCOUNTING_PERIOD_CLOSED", "The applicable accounting period is not open");
        ...
        await tx("journal_entries").insert(input.entries.map((e, index) => ({
            tenant_id: input.tenantId,
...
function validate(input) {
    if (!/^[A-Z]{3}$/.test(input.currency))
        throw new PostingError("CURRENCY_INVALID", "Currency must be ISO 4217 uppercase");
    if (input.entries.length < 2)
        throw new PostingError("POSTING_INVALID", "At least two entries are required");
    for (const e of input.entries)
        if (!["DEBIT", "CREDIT"].includes(e.direction) ||
    ...
    if (total(input.entries, "DEBIT") !== total(input.entries, "CREDIT"))
        throw new PostingError("POSTING_UNBALANCED", "Total debits must equal total credits");
}
function total(entries, direction) {
    return entries
