...
export class EventWorker {
    ...
    async publishOne(channel, exchange = "parc.events") {
        const claimed = await this.db.raw("SELECT * FROM public.claim_next_ledger_outbox(?, ?)", [this.workerId, 30]);
        ...
        return withTenantTransaction(this.db, input.tenantId, async (tx) => {
            ...
            if (existing) {
                if (existing.payload_hash !== payloadHash)
                    throw new Error("Inbox event identity was reused with a different payload");
                return false;
            }
