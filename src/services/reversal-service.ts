...
export class ReversalService {
  ...
  public async reverse(input: {
  ...
  }): Promise<{ reversal_transaction_id: string; replayed: boolean }> {
    if (!input.reason.trim() || (!input.approvalId && !input.automatedRuleId))
      throw new PostingError(
        "REVERSAL_AUTHORIZATION_REQUIRED",
        "A bound approval or pre-approved automated rule is required",
      );
    ...
    if (input.approvalId) {
      if (!this.approvals)
        throw new PostingError(
          ...
          "Manual reversals require the approval service",
        );
      const original = await withTenantTransaction(
        this.database,
        input.tenantId,
        (tx) =>
          tx("ledger_transactions")
            ...
            .first<{ amount: string; currency_code: string }>(),
      );
      if (!original)
        throw new PostingError(
          "TRANSACTION_NOT_REVERSIBLE",
          "Completed transaction is unavailable for reversal",
        );
    ...
    let result: { reversal_transaction_id: string; replayed: boolean };
    try {
      result = await withTenantTransaction(
        ...
        async (tx) => {
          ...
          const existing = await tx("transaction_reversals")
            ...
            .first<{ reversal_transaction_id: string; request_hash: string }>();
          if (existing) {
            if (existing.request_hash !== requestHash)
              throw new PostingError(
                "REVERSAL_ALREADY_EXISTS",
                "Transaction already has a different reversal",
              );
            return {
          ...
          const posted = await this.postings.postInTransaction(tx, {
            tenantId: input.tenantId,
            reference:
              `REV-${original.transaction_reference}-${input.transactionId.slice(0, 8)}`.slice(
                0,
                100,
              ),
            currency: original.currency_code,
