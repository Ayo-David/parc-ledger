...
export class IntegrityWorker {
  ...
  public async runBalanceDrift(
    ...
    return withTenantTransaction(this.db, tenantId, async (tx) => {
      const existing = await tx("ledger_integrity_checks")
        ...
        .first<{ records_failed: number }>();
      if (existing) return { discrepancies: Number(existing.records_failed) };
      const rows = await tx.raw<{
        ...
        `SELECT b.account_id,b.currency_code,
        ...
        WHERE b.tenant_id=? AND b.balance<>CASE WHEN a.normal_balance='DEBIT'
          THEN COALESCE(p.debits-p.credits,0) ELSE COALESCE(p.credits-p.debits,0) END`,
        [tenantId],
      );
      ...
      await tx("ledger_integrity_checks").insert({
        ...
        discrepancy_amount: rows.rows
          .reduce(
            (sum, row) => sum + (BigInt(row.actual) - BigInt(row.expected)),
            0n,
          )
          .toString(),
  ...
  public async recordReconciliationItem(input: {
    tenantId: string;
    runId: string;
    reference: string;
    sourceAmount: string;
    ...
    await withTenantTransaction(this.db, input.tenantId, async (tx) => {
      ...
      const existing = await tx("reconciliation_items")
        ...
        .first<{ source_payload_hash: string }>();
      if (existing) {
      ...
      const difference =
        BigInt(input.sourceAmount) - BigInt(input.ledgerAmount);
