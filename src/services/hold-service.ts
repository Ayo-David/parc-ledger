...
type Entry = PostingCommand["entries"][number];
type HoldResult = {
  hold_id: string;
  account_id: string;
  amount_minor: string;
...
type HoldPayload = Omit<HoldResult, "replayed">;
...
export class HoldService {
  ...
  public async create(input: {
    ...
    if (
      ...
      throw new PostingError(
        "REQUEST_INVALID",
        "Hold currency, purpose, or expiry is invalid",
      );
    ...
    return withTenantTransaction(this.database, input.tenantId, async (tx) => {
      ...
      if (replay)
        return { ...(replay as Omit<HoldResult, "replayed">), replayed: true };
      await tx("ledger_accounts")
        ...
        .then((row) => {
          if (!row)
            throw new PostingError(
              "ACCOUNT_INVALID",
              "Hold account is not active for this tenant and currency",
            );
        });
      ...
      const balance = await tx("ledger_account_balances")
        ...
        .forUpdate()
        .first<{ available_balance: string }>("available_balance");
  ...
  public async capture(input: {
    ...
    reference: string;
    entries: Entry[];
  }): Promise<HoldResult & { transaction_id: string }> {
    return this.act(input, "CAPTURE", async (tx, hold) => {
      const expectedDirection =
        hold.normal_balance === "CREDIT" ? "DEBIT" : "CREDIT";
      const heldAmount = input.entries
        .filter(
          (entry) =>
            entry.accountId === hold.account_id &&
            entry.direction === expectedDirection,
        )
        .reduce((total, entry) => total + BigInt(entry.amountMinor), 0n);
      if (heldAmount !== BigInt(hold.amount))
        throw new PostingError(
          "HOLD_CAPTURE_INVALID",
          "Capture must consume the complete held amount from the reserved account",
        );
  ...
  public async expireDue(tenantId: string, limit = 100): Promise<number> {
  ...
  private async act<T extends HoldPayload>(
    input: {
      tenantId: string;
      holdId: string;
      idempotencyKey: string;
  ...
  ): Promise<T & Pick<HoldResult, "replayed">> {
    ...
    return withTenantTransaction(this.database, input.tenantId, async (tx) => {
      ...
      if (existing) {
        if (existing.request_hash !== requestHash)
          throw new PostingError(
            ...
            "Idempotency key was previously used for a different hold action",
          );
        if (existing.status !== "COMPLETED" || !existing.response_body)
          throw new PostingError(
            "REQUEST_IN_PROGRESS",
            "Hold action is in progress",
          );
      ...
      if (!hold) throw new PostingError("HOLD_NOT_FOUND", "Hold was not found");
...
function validAmount(amount: string): void {
  if (!/^[1-9][0-9]*$/.test(amount))
    throw new PostingError(
      "AMOUNT_INVALID",
      "Amount must be a positive integer minor-unit string",
    );
}
...
async function commandReplay(
  ...
  if (row.request_hash !== requestHash)
    throw new PostingError(
      "IDEMPOTENCY_MISMATCH",
      "Idempotency key was previously used for a different hold request",
    );
  if (row.status !== "COMPLETED" || !row.response_body)
    throw new PostingError(
      "REQUEST_IN_PROGRESS",
      "Hold creation is in progress",
    );
